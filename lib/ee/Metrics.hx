...
abstract Px(Float) {
	...
	public static function fromString( str:String )
	{
		...
		return new Px( ee.Metrics.convert( Std.parseFloat(str), matched ) );
	}
...
class Metrics {
	...
	public static inline function h() {
		var stageH =#if(heaps) hxd.Window.getInstance().height
					#else
	...
	public static inline function w() {
		var stageW =#if(heaps) hxd.Window.getInstance().width
					#else
	...
	public static inline function dpi() : Float {
		//is there a way to get that for heaps ?
